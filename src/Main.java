...
public class Main {
    public static void main(String[] args) {
        ...
        help(instructions);
        while (true) {
            String input = safeInput.SafeInput.getRegExString(in, " ", "[AaDdIiPpHhCcQq]");
    ...
    private static void deleteItem(Scanner in, ArrayList list) {
        if (list.size() > 0) {
            ...
            int indexInput = safeInput.SafeInput.getRangedInt(in, message, 0, listLength);
    ...
    private static boolean quit(Scanner in) {
        return safeInput.SafeInput.getYNConfirm(in, "Are you sure you want to quit?");
    }
